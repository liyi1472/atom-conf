...
function requireHooks(prefix, opts) {
    const globalPnpmfile = opts.globalPnpmfile && requirePnpmfile_1.default(pathAbsolute(opts.globalPnpmfile, prefix));
    let globalHooks = globalPnpmfile && globalPnpmfile.hooks;
    const pnpmFile = opts.pnpmfile && requirePnpmfile_1.default(pathAbsolute(opts.pnpmfile, prefix))
        || requirePnpmfile_1.default(path.join(prefix, 'pnpmfile.js'));
    ...
    if (globalHooks.readPackage || hooks.readPackage) {
        logger_1.default.info('readPackage hook is declared. Manifests of dependencies might get overridden');
    }
